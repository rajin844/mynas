...
class BtrfsDriver(StorageDriver):
    ...
    def create_pool(self, name, layout, dry_run=False):
        devices = layout.get("devices", [])
        if not devices: return {"error":"no devices"}
        if dry_run:
            return {"dry_run": True, "cmd_preview": ["mkfs.btrfs"] + devices}
        res = self._run(["mkfs.btrfs","-f"] + devices)
        if res["rc"] != 0:
            return {"created": False, "error": res["err"]}
        storage = self.cfg.get_storage()
        pools = storage.get("pools", [])
        pools.append({"name": name, "type":"btrfs", "devices": devices})
        self.cfg.update_storage({"pools": pools, "datasets": storage.get("datasets", [])})
        return {"created": True}

    def destroy_pool(self, name, force=False):
        storage = self.cfg.get_storage()
        pools = [p for p in storage.get("pools", []) if p.get("name")!=name]
        self.cfg.update_storage({"pools": pools, "datasets": storage.get("datasets", [])})
        return {"deleted": True}

    def list_datasets(self, pool=None):
        return self.cfg.get_storage().get("datasets", [])

    def create_dataset(self, pool, name, mountpoint=None):
        storage=self.cfg.get_storage()
    ...
    def destroy_dataset(self,pool,name,recursive=False):
        storage=self.cfg.get_storage()
    ...
    def create_snapshot(self,dataset,snapshot_name): return {"error":"not_supported"}
    def destroy_snapshot(self,snapshot): return {"error":"not_supported"}
    ...
    def smart_health(self, devpath):
        try:
            ...
            return {"error": str(e)}
    def ping(self): return {"ok": True, "driver":"btrfs"}
