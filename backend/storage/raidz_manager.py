...
def _group_by_size(devices: List[str]) -> List[str]:
    # naive: return as-is for now, more advanced grouping later
    return devices

def build_raidz_layout(devices: List[str], mode: str = "single") -> Dict[str, Any]:
    """
    Return layout: {"vdevs": [{"type":"mirror","disks":[...]} ...]}
    modes: single, mirror, raidz1, raidz2, raidz3
    """
    mode = (mode or "single").lower()
    devs = _group_by_size(devices)
    if mode == "single":
        return {"vdevs": [{"type": "single", "disks": [[d] for d in devs]}]}
    if mode == "mirror":
        if len(devs) < 2:
            raise ValueError("mirror needs at least 2 devices")
        return {"vdevs": [{"type": "mirror", "disks": [devs]}]}
    if mode.startswith("raidz"):
        # For simplicity place all disks in one raidz vdev
        return {"vdevs": [{"type": mode, "disks": [devs]}]}
    raise ValueError("Invalid raidz mode")        
